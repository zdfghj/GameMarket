...
import com.download.manager.DownloadRequestCallBack;
	...
	private class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				switch (itemViewType) {
				case KEY_1:
					convertView = LayoutInflater.from(DownloadList.this)
							.inflate(R.layout.activity_downloadlist_downloadingtab, null);
					holder = new Hodler1();
					((Hodler1) holder).act_downloadlist_tv_ing = (TextView) convertView
							.findViewById(R.id.act_downloadlist_tv_ing);
					break;
				case KEY_2:
					convertView = LayoutInflater.from(DownloadList.this)
							.inflate(R.layout.activity_downloadlist_downloadfinishtab, null);
					holder = new Hodler2();
					((Hodler2) holder).act_download_tv_finish = (TextView) convertView
							.findViewById(R.id.act_download_tv_finish);
					((Hodler2) holder).act_download_tv_clean = (TextView) convertView
							.findViewById(R.id.act_download_tv_clean);
					// 清空记录操作
				...
				case KEY_3:
					...
					((Hodler3) holder).download_remove_btn = (Button) convertView
							.findViewById(R.id.download_remove_btn);
			...
			case KEY_3:
				...
				if (position < downloadingInfos.size() + 1) {// 下载中
					...
					final DownloadInfo downloadInfo = downloadFinishInfos
							.get(position - downloadingInfos.size() - 1 - 1);
					holder.update(downloadInfo);
					setHolderData((Hodler3) holder, downloadInfo, downloadManager);
				}
				break;
		 ...
		 * @param downloadInfo
		 */
		private void setHolderData(final Hodler3 holder, final DownloadInfo downloadInfo,
					...
					public void onClick(View v) {
						try {
							if (downloadInfo.getState() == HttpHandler.State.CANCELLED
									|| downloadInfo.getState() == HttpHandler.State.FAILURE) {
